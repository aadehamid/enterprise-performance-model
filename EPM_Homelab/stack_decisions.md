...
- `/home/user/workspace/research_oss_tool_stack.md` — full tool license/maintenance research with citations (triple stores, Neo4j/RDF bridges, R2RML/RML, SHACL, ontology authoring, Python RDF libs, agent frameworks, local LLMs, orchestration, OLAP/OLTP databases, lineage, SKOS/PROV-O, Google OKF).
- `/home/user/workspace/research_public_datasets.md` — full public dataset and synthetic-data-tooling research with citations (EIA, EPA, PHMSA, BTS, NOAA, Texas RRC, Kaggle, World Bank, IMF, 3W, DWSIM, Faker, Mimesis, SDV, TimeSynth, PIDX, and synthetic-data generation approaches for custody transfer/BOL data, SCADA/historian tags, and O2C relational schemas).
