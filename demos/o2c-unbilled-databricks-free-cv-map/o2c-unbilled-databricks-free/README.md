...
Two new objects: `dim_kpi_metadata` (Silver, one row per KPI, all
certified) and `gold_kpi_value` (Gold, published instances per
certified KPI FROM that view's `MEASURE()`). Both live in
`sql/05_store.sql` / `scripts/06_store.py`. After a full run the
catalog has three certified rows and Gold has all three KPIs.
...
is the ad hoc path for all three certified KPIs (MEASURE() only;
...
Prove the warehouse, then Act 0 ODS → replica → facts → metric view → queries → store → KPI 2 → KPI 3 → Genie → CV/map → hunt → vocabulary-search Genie:
...
| `06_store.py` | Unbilled row in `dim_kpi_metadata` (`certified`, pointer `unbilled_usd`); Unbilled gold FROM `MEASURE()` (11 rows); enterprise **$179,934.00** / 12. Other kpi_id gold/metadata rows are kept. After 08/09 the catalog is 3 certified rows. |
...
| `07_genie.py` | Run **after** 08/09. Creates/updates Genie Agent **O2C certified KPIs** on all three Metric Views; live-asks unsliced Unbilled / contract / temp-adjusted. MEASURE() only. No Export-to-metric-view. |
...
| Store | `sql/05_store.sql` | Unbilled metadata + Unbilled gold FROM `MEASURE()`; enterprise `$179,934.00`. After 08/09: 3 certified catalog rows and gold for all three KPIs. |
...
writes SQL for a complex formula. They are **not** a second Unbilled. Each is `certified` and publishes
...
o2c-unbilled-databricks-free/
  ...
  scripts/07_genie.py       # Genie Agent; run AFTER 08/09; MEASURE() on all three certified views
...
| Genie answers from `fct_unbilled` or `raw_*` | Fail. Agent must MEASURE() the matching certified view (unbilled / contract / temp-adjusted). |
